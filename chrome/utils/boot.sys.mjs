...
class ScriptData {
  ...
  constructor(leafName, headerText, noExec, isStyle) {
    ...
    this.inbackground =
      this.filename.endsWith('.sys.mjs') || /\/\/ @backgroundmodule\b/.test(headerText);
    ...
    if (this.inbackground || this.styleSheetMode === 'agent' || (!isStyle && noExec)) {
      ...
      while ((match = findNextRe.exec(headerText))) {
        rex[match[1]].push(match[2].replace(/^main$/i, BROWSERCHROME).replace(/\*/g, '.*?'));
      }
  ...
  static registerScriptManifest(aScript) {
    ...
    if (cmanifest.isFile()) {
      Components.manager.QueryInterface(Ci.nsIComponentRegistrar).autoRegister(cmanifest.entry());
    } else {
      console.warn(
...
class UserChrome_js {
  ...
  init() {
    ...
    if (styleDir.isDirectory()) {
      ...
      this.addAgentStyles(
        this.styles.filter(
          (style) => style.styleSheetMode === 'agent' && !disabledScripts.includes(style.filename)
        )
      );
    }
  ...
  generateScriptMenuItemsIfNeeded(aDoc) {
    ...
    aDoc.l10n
      .formatValues(['restart-button-label', 'clear-startup-cache-label', 'show-dir-label'])
      .then((values) => {
