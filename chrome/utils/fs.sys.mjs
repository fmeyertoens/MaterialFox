export class FileSystem {
  ...
  static convertChromeURIToFileURI(aURI) {
    ...
    return registry.convertChromeURL(aURI instanceof Ci.nsIURI ? aURI : Services.io.newURI(aURI));
  }
  ...
  static {
    this.#RESOURCE_URI = FileSystem.getFileURIForFile(
      FileSystem.convertChromeURIToFileURI('chrome://userchrome/content/').QueryInterface(
        Ci.nsIFileURL
      ).file.parent,
      FileSystem.RESULT_DIRECTORY
    );
    this.#SCRIPT_URI = FileSystem.getFileURIForFile(
      FileSystem.convertChromeURIToFileURI('chrome://userscripts/content/').QueryInterface(
        Ci.nsIFileURL
      ).file.parent,
      FileSystem.RESULT_DIRECTORY
    );
    this.#STYLE_URI = FileSystem.getFileURIForFile(
      FileSystem.convertChromeURIToFileURI('chrome://userstyles/skin/').QueryInterface(
        Ci.nsIFileURL
      ).file.parent,
      FileSystem.RESULT_DIRECTORY
    );
  ...
  static #getEntryFromString(aFilename, baseFileURI) {
    ...
    if (typeof aFilename !== 'string') {
      return FileSystemResult.fromErrorKind(FileSystem.ERROR_KIND_INVALID_ARGUMENT, {
        expected: 'String',
      });
    }
    ...
    if (aFilename instanceof Ci.nsIURI) {
      if (aFilename.scheme === 'chrome') {
        return FileSystemResult.fromNsIFile(
          FileSystem.convertChromeURIToFileURI(aFilename).QueryInterface(Ci.nsIFileURL).file
        );
      }
  ...
  static readNSIFileSyncUncheckedWithOptions(aFile, options) {
    ...
    try {
      ...
      cvstream.close();
      stream.close();
      return FileSystemResult.fromErrorKind(FileSystem.ERROR_KIND_NOT_READABLE, {
        cause: e,
  ...
  static readIni(path) {
    let fileURI = FileSystem.convertChromeURIToFileURI(
      Services.io.newURI(`chrome://userchrome/content/${path}`)
    );
  ...
  static IO = new Proxy(IOUtils, {
    get(target, prop) {
      ...
      return (path, ...args) =>
        Reflect.apply(func, null, [FileSystem.#appendToBaseURI(path), ...args]);
    },
  });
  ...
  static #parse(ini, fileURI) {
    ...
    while (sections.hasMore()) {
      ...
      for (let iniKey of Array.from(ini.getKeys(section))) {
        ...
        uniqueKeys.set(leafName, IniUtils.#iniValue(ini.getString(section, iniKey).trim()));
      }
...
class ResultError extends Error {
  ...
  static fromKind(aKind, info) {
    return info instanceof ResultError ? info : new ResultError(aKind, {}, info);
  }
}
...
class FileSystemResult {
  ...
  content(replaceNewlines) {
    if (this.isContent()) {
      return replaceNewlines ? this.#result.content.replace(/\r\n?/g, '\n') : this.#result.content;
    }
    throw ResultError.fromKind(FileSystem.ERROR_KIND_NOT_CONTENT, { type: this.#type.description });
  }
  get size() {
    return this.isContent() ? this.#result.content.length : this.#result.fileSize;
  }
  entry() {
  ...
  static #generateErrorInfo(aResult) {
    ...
    return {
      topic: aResult.isContent() ? aResult.#result.path : aResult.#result.leafName,
    };
  }
  ...
  static fromNsIFile(entry) {
    ...
    return FileSystemResult.fromErrorKind(FileSystem.ERROR_KIND_UNKNOWN_RESULT, {
      topic: entry.leafName,
    });
  }
}
