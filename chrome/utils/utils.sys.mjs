...
export class Hotkey {
  ...
  get matchingSelector() {
    if (!this.#matchingSelector) {
      ...
      this.#matchingSelector = `key[modifiers="${trigger.modifiers}"][${
        trigger.key ? 'key="' + trigger.key : 'keycode="' + trigger.keycode
      }"]`;
    }
  ...
  static define(desc) {
    ...
    return new Hotkey(
      keyDetails,
      commandType === 'function' ? { id: keyDetails.command, command: desc.command } : null
    );
  }
}
...
function reloadStyleSheet(name, type) {
  ...
  let sheets = recentWindow.InspectorUtils.getAllStyleSheets(recentWindow.document, false).flatMap(
    (x) => recurseImports(x, [x])
  );
...
export const loaderModuleLink = new (function () {
  ...
  Object.defineProperty(this, 'brandName', {
    get: () => {
      if (brandName === null) {
        brandName = ChromeUtils.importESModule('resource://gre/modules/AppConstants.sys.mjs')
...
export class windowUtils {
  ...
  static #observe(aSubject) {
    aSubject.addEventListener('DOMContentLoaded', windowUtils.#onDOMContent, { once: true });
  }
  ...
  static isBrowserWindow(window) {
    return (
      window.document.documentElement.getAttribute('windowtype') === windowUtils.mainWindowType
    );
  }
      ...
      if (typeof callback === 'function') {
        if (desc.allEvents) {
          ...
          toolbaritem.addEventListener(
            'click',
            (ev) => ev.button === 0 && WidgetCallbacks.get(ev.target.id)(ev, ev.target.ownerGlobal)
          );
        }
      }
...
function getScriptInfoForType(aFilter, aScriptList) {
  ...
  if (filterType === 'function') {
    return aScriptList
      .filter(aFilter)
      .map((script) => ScriptInfo.fromScript(script, !disabledScripts.includes(script.filename)));
  }
...
export function toggleScript(aFilename) {
  ...
  let script = aFilename.endsWith('js') ? getScriptData(aFilename) : getStyleData(aFilename);
  if (!script) {
    return null;
  }
  ...
  if (isEnabled) {
    ...
    Services.prefs.setCharPref(
      ...
      prefValue.replace(new RegExp(`^${script.filename},?|,${script.filename}`), '')
    );
  }
